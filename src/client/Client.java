...
public class Client {
    public static void main(String[] args) {
        try {
            Socket socket = new Socket(/*"152.70.168.210"*/"localhost", 8188);
            ...
            Thread thread = new Thread(() -> {
                ...
                while (true){ // Бесконечно ожидаем сообщение от сервера
                    try {
                        response1 = in.readUTF(); // Читаем ответ от сервера
                        System.out.println(response1); // Печатаем ответ от сервера на экран
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
